...
DO d=1,36,1 !for each wdir (c/10 grados)
        !write(*,'("      Wind flow passing", I4," degree direction.")') d*10 
        wdir=(360-d*10)*2*pi/360.0   !wdir [rad]
        ...
        DO i=1,size(S),1 !for each stack
                ...
                DO j=1,size(B),1 !for each building
                    DO k=1,size(B(j)%t),1 !for each tier
                       ...
                       call setTierProyectedBoundary(B(j)%T(k))
                       call setTierValues(B(j)%T(k)) 
                       ...
                       if ( isInsideL5(S(i),B(j)%T(k)) .AND. isInsideSIZ(S(i), B(j)%T(k)) ) then
                               !check si hay otra structura cercana (de mismo tier) y combinarlos:    
                                ...
                                !call setTierValues(B(j)%T(k))                     !calc: WID LEN HGT L
        
                                call calcGepStackHeight(B(j)%z0, B(j)%T(k), S(i)) !calc: GSH, XBADJ, YBADJ
                                !print*,S(i)%nombre,B(j)%nombre  
                       else 
...
function isInsideL5(S,T)   result(L5)
        ...
        L5=( minDIST( S%xy, T%xy ) .LT. 5.0*T%L )
end function
function isInsideSIZ(S,T)       result(SIZ)
        ...
        type(tier), intent(in)  :: T
        logical :: SIZ
        !  Stack está dentro del SIZ de la estructura?  SIZ= [xmin-0.5*L - xmax + 0.5*L] , [ymax+5L - ymin -2L]
        SIZ=(          S%xy2(1,1) .GE. T%xmin - 0.5*T%L )                                          
        SIZ=(SIZ .AND. S%xy2(1,1) .LE. T%xmax + 0.5*T%L )
        SIZ=(SIZ .AND. S%xy2(1,2) .GE. T%ymin - 2.0*T%L )
endfunction
...
subroutine rotar(B,S,wdir)
        ...
        !stack
        do i=1,size(S),1
                S(i)%xy2(1,:)=sngl(matmul(R,dble(S(i)%xy(1,:))))  !proyected coords stack
        enddo
        !buildings
        do i=1,size(B),1
                do j=1,size(B(i)%t),1
                        B(i)%t(j)%xy2(:,1:2)=transpose(  matmul(R,dble(transpose(B(i)%t(j)%xy(:,1:2))) ) ) !proyected coords tier
                        call setTierProyectedBoundary(B(i)%T(j))
...
subroutine setTierProyectedBoundary(T)
        ...
        T%xmin=sngl(minval(T%xy2(:,1)) ) 
        T%xmax=sngl(maxval(T%xy2(:,1)) )
        T%ymin=sngl(minval(T%xy2(:,2)) )
        T%ymax=sngl(maxval(T%xy2(:,2)) )
end subroutine
...
subroutine calcGepStackHeight(Bz0,t,s)!xy,xy2,h,
        ...
        T%xbadj= T%ymin - s%xy2(1,2)                !XBADJ = YMIN(C) - YPSTK                 !ymin - ystack
        T%ybadj= s%xy2(1,1) - (T%xmin + T%wid*0.5)  !YBADJ = XPSTK - (XMIN(C) + TW * 0.5)    !xstack- xmin + tw*0.5
end subroutine
